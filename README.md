...
API RESTful de gerenciamento de produtos — CRUD completo, filtros avançados, autenticação JWT, auditoria assíncrona via fila e busca full-text com Elasticsearch.

Desenvolvida em Laravel 13 para o desafio técnico "Desenvolvedor(a) Back-End Sênior".
...
| Busca | Elasticsearch 9 (`elasticsearch/elasticsearch`) |
| Autenticação | JWT (`tymon/jwt-auth`) |
| Documentação | OpenAPI 3 / Swagger (`darkaonline/l5-swagger`) |
| Infraestrutura | Docker Compose — PHP-FPM, Nginx, Postgres, Elasticsearch, worker de fila |
| Testes | PHPUnit |
...
Aceita um token já expirado, desde que dentro do `refresh_ttl`.
...
#### `GET` `/api/v1/products` — Listar com filtros e paginação
...
Aceita atualização parcial — envie só os campos que quer mudar.
